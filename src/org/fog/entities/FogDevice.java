...
//meeeeeeeeeeeeeeeee
import iFogSim.AdaptiveAllocatorModule;
import org.fog.application.AppModule;
...
public class FogDevice extends PowerDatacenter {
    ...
    // In FogDevice.java
    // In FogDevice.java
    // In FogDevice.java, replace the checkCloudletCompletion method
            ...
            for (Vm vm : host.getVmList()) {
                while (vm.getCloudletScheduler().isFinishedCloudlets()) {
                    ...
                    if (cl != null) {
                        ...
                        String moduleName = ((AppModule)vm).getName();

                        if (tuple.getTupleType().startsWith("CONTEXT_TUPLE")) {
                            if (moduleName.equals("adaptive-allocator-module")) {
                                ((iFogSim.AdaptiveAllocatorModule)vm).onTupleArrival(tuple);
                            }
                        } else if (tuple.getTupleType().equals("V2V_ALERT")) {
                            if (moduleName.equals("alert-receiver")) {
                                System.out.println("Alert received by " + moduleName + " on " + getName() + ": WARNING from nearby vehicle!");
                            }
                        }
            ...
            for (Vm vm : host.getVmList()) {
                while (vm.getCloudletScheduler().isFinishedCloudlets()) {
                    ...
                    if (cl != null) {
                        ...
                        // --- START OF CUSTOM CODE BLOCK TO ADD ---
                        // This code checks if the tuple is the one we care about and calls our custom logic.
                        // NOTE: You may need to add 'import iFogSim.AdaptiveAllocatorModule;' to the top of this file.
                        // --- NEW, CORRECTED BLOCK ---
                        if (tuple.getTupleType().equals("CONTEXT_TUPLE")) {
                            // Check the module's name instead of its class type
                            if (((AppModule)vm).getName().equals("adaptive-allocator-module")) {
                                ((iFogSim.AdaptiveAllocatorModule)vm).onTupleArrival(tuple);
                            }
                        }
